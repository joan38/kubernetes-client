  ...
  private def mkSecureContext[F[_]: Concurrent: Files](config: KubeConfig[F]): F[Option[SecureContext]] =
    for {
      ...
      if (keyBytes.nonEmpty || certBytes.nonEmpty || caBytes.nonEmpty)
        SecureContext(
          ca =
            caBytes.map(caBytes => Seq(caBytes.asLeft[String])), // : Option[Seq[Either[Chunk[Byte], String]]] = None,
          cert = certBytes.map(certBytes =>
            Seq(certBytes.asLeft[String])
          ), // : Option[Seq[Either[Chunk[Byte], String]]] = None,
